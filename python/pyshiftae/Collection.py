...
class ItemCollection(list):
    ...
    def append(self, object: Item) -> None:
        if (
            PyFx.ItemSuite().GetItemParentFolder(object.item)
            != self._ROOT_FOLDER
        ):
            PyFx.ItemSuite().SetItemParentFolder(
...
class LayerCollection(list):
    ...
    def add_null(self, name: str, duration: float = 0.0) -> Layer:
        if duration < 0:
            duration = PyFx.ItemSuite().GetItemDuration(
    ...
    def add_solid(
        ...
        solid = PyFx.CompSuite().CreateSolidInComp(
            ...
            width,
            height,
            PyFx.ColorVal.from_tuple(color),
            duration,
        )
    ...
    def append(self, item: Item) -> None:
        if PyFx.LayerSuite().IsAddLayerValid(item.item, self._base_comp):
            layer = Layer(
                PyFx.LayerSuite().AddLayer(item.item, self._base_comp)
            )
            super().append(layer)
    ...
    def insert(self, index: int, layer: Layer) -> None:
        if PyFx.LayerSuite().IsAddLayerValid(
            layer.source_item, self._base_comp
        ):
            new_layer = Layer(
                PyFx.LayerSuite().AddLayer(layer.source_item, self._base_comp)
            )

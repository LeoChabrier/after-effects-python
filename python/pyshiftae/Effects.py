...
class Effect:
    ...
    def param(self, name: Union[int, str]) -> Union[
        ...
        if isinstance(name, int):
            stream = PyFx.StreamSuite().GetNewEffectStreamByIndex(
                self.effect, name
            )
            return PropertyFactory.create_property(stream)
        else:
            for i in range(
                PyFx.StreamSuite().GetEffectNumParamStreams(self.effect)
            ):
                current_name = PyFx.StreamSuite().GetStreamName(
                    PyFx.StreamSuite().GetNewEffectStreamByIndex(
                        self.effect, i
                    ),
                    True,
                )
                current_match_name = PyFx.DynamicStreamSuite().GetMatchname(
                    PyFx.StreamSuite().GetNewEffectStreamByIndex(self.effect, i)
                )

...
class Layer(PropertyGroup):
    def __init__(self, layer: PyFx.LayerPtr) -> None:
        super().__init__(
            PyFx.DynamicStreamSuite().GetNewStreamRefForLayer(layer)
        )
        self.layer = layer
    ...
    def source_item(self) -> Item:
        return ItemFactory.create_item(
            PyFx.LayerSuite().GetLayerSourceItem(self.layer)
        )
    ...
    def video_active(self) -> bool:
        return (
            PyFx.LayerSuite().GetLayerFlags(self.layer)
            & PyFx.LayerFlag.VIDEO_ACTIVE
        )
    ...
    def parent(self) -> "Layer":
        return LayerFactory.create_layer(
            PyFx.LayerSuite().GetLayerParent(self.layer)
        )
    ...
    def duplicate(self) -> "Layer":
        return LayerFactory.create_layer(
            PyFx.LayerSuite().DuplicateLayer(self.layer)
        )
    ...
    def get_property(self, stream: PyFx.LayerStream) -> Union[
        ...
        return PropertyFactory.create_property(
            PyFx.StreamSuite().GetNewLayerStream(self.layer, stream)
        )
    ...
    def copy_to_comp(self, comp: CompItem) -> "Layer":
        if PyFx.LayerSuite().IsAddLayerValid(self.source_item.item, comp.comp):
            return LayerFactory.create_layer(
                PyFx.LayerSuite().AddLayer(self.source_item.item, comp.comp)
            )

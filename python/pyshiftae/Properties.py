...
class BaseProperty:
    ...
    def duplicate(
        ...
        stream = PyFx.DynamicStreamSuite().GetNewStreamRefByIndex(
    ...
    def get_property(self, name: str) -> Union[
        ...
        stream = PyFx.DynamicStreamSuite().GetNewStreamRefByMatchname(
            self.property, name
        )
        return PropertyFactory.create_property(stream)
...
class OneDProperty(BaseProperty):
    ...
    def get_value(
        ...
        val = PyFx.StreamSuite().GetNewStreamValue(
            self.property, time_mode, PyFx.Time(time), pre_expression
        )
        return val.value()

    def set_value(self, value: float) -> None:
        PyFx.StreamSuite().SetStreamValue(
            self.property, PyFx.StreamValue2Ptr(self.property, value)
        )
